...
import { Package, Clapperboard, Gem, Layers, Globe, ShoppingCart, Search } from "lucide-react"
...
export function ServicesSection() {
  ...
  const services3d = [
    {
      icon: Package,
      titleKey: "services3d.capability.products.title",
      descKey: "services3d.capability.products.desc",
    },
    {
      icon: Clapperboard,
      titleKey: "services3d.capability.animation.title",
      descKey: "services3d.capability.animation.desc",
    },
    {
      icon: Gem,
      titleKey: "services3d.capability.branding.title",
      descKey: "services3d.capability.branding.desc",
    },
    {
      icon: Layers,
      titleKey: "services3d.capability.cgi.title",
      descKey: "services3d.capability.cgi.desc",
    },
  ]
  ...
  const renderCard = (service: { icon: LucideIcon; titleKey: string; descKey: string }, index: number, is3D = false) => {
    const Icon = service.icon
    return (
      <Card
        key={index}
        className={`group glass-card-hover hover:border-primary/40 relative overflow-hidden transition-all duration-700 bg-card border-border ${is3D ? "min-h-[280px]" : ""}`}
        style={{
          ...
          <div>
            <div className="mb-6 flex">
              <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 group-hover:bg-primary/10 group-hover:border-primary/30 transition-all duration-500 group-hover:scale-110">
                <Icon className="w-7 h-7 text-primary transition-all duration-500 group-hover:scale-105" />
              </div>
            </div>
  ...
  return (
    <section className="relative py-24 bg-muted/10 overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <StaggeredReveal staggerDelay={100} direction="up" className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {services3d.map((service, index) => renderCard(service, index, true))}
        </StaggeredReveal>
        ...
        <ScrollReveal direction="up" delay={0} duration={1000}>
          <div className="flex items-center justify-center gap-4 mb-10">
            ...
            <span className="text-sm uppercase tracking-widest font-sans font-semibold text-primary">
              {t("services.complement.tag")}
            </span>
            <span className="h-px w-16 bg-gradient-to-l from-transparent to-primary/40" />
          </div>
        </ScrollReveal>

        <StaggeredReveal staggerDelay={120} direction="up" className="grid md:grid-cols-3 gap-8">
          {complementary.map((service, index) => renderCard(service, index))}
        </StaggeredReveal>
      </div>
    </section>
  )
